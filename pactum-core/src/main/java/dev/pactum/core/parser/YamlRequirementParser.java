...
public class YamlRequirementParser implements RequirementParser {
    ...
    @Override
    public ModuleContract parse(Path requirementYaml) throws PactumException {
        Map<String, Object> root;
        try (InputStream in = Files.newInputStream(requirementYaml)) {
            Yaml yaml = new Yaml();
            root = yaml.load(in);
        } catch (IOException e) {
            throw new PactumException("Failed to read " + requirementYaml, e);
        }

        if (root == null) {
            throw new PactumException("Empty YAML file: " + requirementYaml);
        }
    ...
    private ModuleInfo parseModule(Map<String, Object> root) throws PactumException {
        Object moduleObj = root.get("module");
        if (!(moduleObj instanceof Map)) {
            throw new PactumException("Missing or invalid 'module' section");
        }
    ...
    @SuppressWarnings("unchecked")
    private List<RequirementDefinition> parseRequirements(Map<String, Object> root, String moduleId) throws PactumException {
        Object reqsObj = root.get("requirements");
        if (reqsObj == null) {
            return Collections.emptyList();
        }
        if (!(reqsObj instanceof List)) {
            throw new PactumException("'requirements' must be a list");
        }

        List<RequirementDefinition> result = new ArrayList<>();
        for (Object item : (List<?>) reqsObj) {
            if (!(item instanceof Map)) {
                throw new PactumException("Each requirement must be a map");
            }
    ...
    @SuppressWarnings("unchecked")
    private RequirementDefinition parseRequirement(Map<String, Object> m, String moduleId) throws PactumException {
    ...
    private void validateRequirementId(String id, String moduleId) throws PactumException {
        if (!REQ_ID_PATTERN.matcher(id).matches()) {
            throw new PactumException("Invalid requirement ID format: '" + id
                    + "'. Expected {MODULE}-{SEQ} (e.g. " + moduleId + "-001)");
        }
    }

    private void validateCornerCaseId(String id, String moduleId) throws PactumException {
        if (!CC_ID_PATTERN.matcher(id).matches()) {
            throw new PactumException("Invalid corner case ID format: '" + id
    ...
    private String requireString(Map<String, Object> map, String key, String context) throws PactumException {
        Object value = map.get(key);
        if (value == null) {
            throw new PactumException("Missing required field '" + key + "' in " + context);
        }
